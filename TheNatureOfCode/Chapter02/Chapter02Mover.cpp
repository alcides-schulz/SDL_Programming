...
Chapter02Mover::Chapter02Mover(SDL_Framework *framework, PVector location, int mass) :
    framework(framework), location(location), mass(mass) {
...
{
    if (location.x > framework->WindowWidth() - radius) {
        location.x = (float)framework->WindowWidth() - radius;
        velocity.x *= -1;
    }
    ...
    if (location.y > framework->WindowHeight() - radius) {
        location.y = (float)framework->WindowHeight() - radius;
        velocity.y *= -1;
    }
